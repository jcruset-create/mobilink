...
│   │   ├── importes.ts       «‑45,63», «272.83e», «1.234,56» → céntimos
...
albaran_solicitado TEXT,               -- tal cual venía en el correo («0806295»)
...
numero_documento TEXT,                  -- raw_pdf_delivery_note («ENT-100126-0806295»)
...
2. **Heurística de actuaciones** (`actuaciones.ts`, confianza 0.90–0.95):
   ...
   **acción vigente**: una línea que es solo un verbo de acción (`Grabar`,
   ...
   misma línea se descompone en **importe** (`272.83e`, `17,87 €`, `137.08€`)
...
(`importes.ts` admite `‑45,63`, `-45.63`, `272.83e`, `1.234,56 €`, `(45,63)`),
...
1. **«Acción incompatible» no resta cuando el albarán coincide.** Con la
   tabla literal, `GRABAR 2028359553` seguido de `MODIFICAR 2028359553` daría
...
Cada sumando se anota en `motivos[]` con texto («misma factura 0000123514»,
...
```
normalizarAlbaran("ENT-100126-0806295") → { completo: "ENT1001260806295", nucleo: "806295", segmentos: ["ENT","100126","0806295"] }
normalizarAlbaran("0806295")            → { completo: "0806295",           nucleo: "806295", segmentos: ["0806295"] }
```
...
| el solicitado es un segmento entero del documento (`0806295` ∈ `ENT-100126-0806295`) | 0.92 | MATCH |
...
0.60). **Dos albaranes parecidos** (`0806295` y `0806296`) no se confunden:
...
4. **Comprobación aritmética por línea** (`cuadra_aritmetica`):
   ...
   número mal leído (§52): un `81,10` leído como `81,16` deja de cuadrar.
...
Con el fixture generado con esos valores: sección `ENT-100126-0806295`
(MATCH 0.92 por segmento), 4 líneas, `29,20 + 210,00 + 33,62 + 63,36 =
336,18`, cada línea cuadra (`81,10 × 0,4 × 0,9 = 29,196 → 29,20`), matrícula
`6352GVV`, bastidor `VF30E9HZHAS115406`, observación `ENTREGAR EN TGNA.`,
`diferencia = +63,35` frente a `272,83` → validación `IMPORTE` en REVISAR con
...
Cabecera: número, título derivado (`GRABAR ALBARÁN 2028359553` o `3
...
- `correo/plantilla`, `correo/actuaciones`, `correo/importes`,
  ...
  línea, prosa, `272.83e T2`, ambiguos).
...
descuentos encadenados, albaranes `0806295`/`0806296`, referencia ilegible,
...
| Caso | Comprobación |
|---|---|
| 1 | 1 expediente, 1 actuación `GRABAR 2028359553`, importe `‑4563`, cola con 1 fila |
...
| 12 | `0806295` vs `ENT-100126-0806295` → MATCH ≥ 0.9 |
| 13 | `0806295` y `0806296` en el PDF → se coge el correcto; el otro anotado como parecido |
...
- **La normalización de identificadores está escrita DOS veces**, en
  ...
  tiene que cruzar `0000123514` con `123514` sin traerse a Node los

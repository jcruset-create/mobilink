...
const OPERATOR_STATUS: Record<string, { label: string; cls: string }> = {
...
/** Tarjeta de operario: nombre, rol, estado y teléfono con botón de llamada. */
function TarjetaOperario({ op }: { op: Operator }) {
  const st = op.status ? OPERATOR_STATUS[op.status] ?? { label: op.status, cls: "border-slate-600 text-slate-400" } : null;
  return (

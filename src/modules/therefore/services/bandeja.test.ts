...
const actuacion = (sobre: Partial<Actuacion> = {}): Actuacion => ({
  ...
  expedienteId: "e1",
  tipoAccion: "GRABAR",
  albaranSolicitado: "2028359553",
  albaranNormalizado: "2028359553",
...
describe("texto de una actuación", () => {
  it("es la acción y el albarán", () => {
    expect(textoActuacion(actuacion())).toBe("GRABAR 2028359553");
  });
  ...
  it("enseña el indicador que no se sabe interpretar", () => {
    expect(textoActuacion(actuacion({ indicadorAdicional: "T2" }))).toBe("GRABAR 2028359553 (T2)");
  });

    ...
    <CoreLayout>
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-400">{filtrados.length} empresas</p>
        </div>
      ...
      <input value={filtro} onChange={(e) => setFiltro(e.target.value)}
        placeholder="Buscar por nombre, CIF, sector, ciudad..." className="rounded-lg border border-slate-700 px-3 py-2 text-sm w-72" />
            ...
            <tbody>
              {filtrados.map((e) => (
                <tr key={e.id} className="border-t border-slate-700 hover:bg-slate-700/50">
                  ...
                  <td className="p-3 text-slate-400 font-mono text-xs">{e.cif ?? "—"}</td>
                  <td className="p-3 text-slate-400">{e.sector ?? "—"}</td>
                  <td className="p-3 text-slate-400">
                    <div>{e.email ?? "—"}</div>
                    <div className="text-xs">{e.telefono ?? ""}</div>
                  </td>
                  <td className="p-3 text-slate-400">{e.ciudad ?? "—"}</td>
                  <td className="p-3">
                    <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${e.activa ? "bg-green-100 text-green-800" : "bg-red-500/20 text-red-300"}`}>
                      {e.activa ? "Activa" : "Inactiva"}
                    </span>
                  </td>
                  <td className="p-3">
                    <button onClick={() => abrirEditar(e)} className="rounded-lg bg-slate-700 px-2 py-1 text-xs hover:bg-slate-600">Editar</button>
                  </td>
                </tr>
              ))}
              {filtrados.length === 0 && <tr><td colSpan={7} className="p-8 text-center text-slate-500">Sin empresas.</td></tr>}
            </tbody>
          </table>
        </div>
      )}
          ...
          <div className="w-full max-w-lg rounded-2xl bg-slate-800 p-6 shadow-xl">
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div><label className="text-xs font-medium text-slate-300">CIF / NIF</label>
                  <input value={form.cif} onChange={(e) => setForm({ ...form, cif: e.target.value })}
                    className="mt-1 w-full rounded-lg border border-slate-600 bg-slate-900 px-3 py-2 text-sm text-slate-100 placeholder:text-slate-500 outline-none focus:ring-2 focus:ring-sky-500" /></div>
                <div><label className="text-xs font-medium text-slate-300">Sector</label>
                  <input value={form.sector} onChange={(e) => setForm({ ...form, sector: e.target.value })}
                    className="mt-1 w-full rounded-lg border border-slate-600 bg-slate-900 px-3 py-2 text-sm text-slate-100 placeholder:text-slate-500 outline-none focus:ring-2 focus:ring-sky-500" placeholder="Industria, Logística..." /></div>
              </div>
              <div className="grid grid-cols-2 gap-3">
              ...
              <div><label className="text-xs font-medium text-slate-300">Dirección</label>
                <input value={form.direccion} onChange={(e) => setForm({ ...form, direccion: e.target.value })}
                  className="mt-1 w-full rounded-lg border border-slate-600 bg-slate-900 px-3 py-2 text-sm text-slate-100 placeholder:text-slate-500 outline-none focus:ring-2 focus:ring-sky-500" /></div>
              <div><label className="text-xs font-medium text-slate-300">Ciudad</label>

    ...
    <CoreLayout>
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-slate-400">{filtrados.length} centros</p>
        </div>
      ...
      <input value={filtro} onChange={(e) => setFiltro(e.target.value)}
        placeholder="Buscar por nombre, código, ciudad..." className="rounded-lg border border-slate-700 px-3 py-2 text-sm w-72" />
            ...
            <tbody>
              {filtrados.map((c) => (
                <tr key={c.id} className="border-t border-slate-700 hover:bg-slate-700/50">
                  ...
                  <td className="p-3 font-mono text-xs text-slate-400">{c.codigo ?? "—"}</td>
                  <td className="p-3 text-slate-400">{(c.sea_companies as any)?.nombre ?? "—"}</td>
                  <td className="p-3 text-slate-400">{c.ciudad ?? "—"}</td>
                  <td className="p-3 text-slate-400">
                    <div>{c.email ?? "—"}</div>
                    <div className="text-xs">{c.telefono ?? ""}</div>
                  </td>
                  <td className="p-3">
                    <span className={`rounded-full px-2 py-0.5 text-xs font-medium ${c.activo ? "bg-green-100 text-green-800" : "bg-red-500/20 text-red-300"}`}>
              ...
              {filtrados.length === 0 && <tr><td colSpan={7} className="p-8 text-center text-slate-500">Sin centros.</td></tr>}
            </tbody>
          </table>
        </div>
      )}
          ...
          <div className="w-full max-w-lg rounded-2xl bg-slate-800 p-6 shadow-xl">
            ...
            <div className="space-y-3">
              <div className="grid grid-cols-2 gap-3">
                ...
                <div><label className="text-xs font-medium text-slate-300">Código interno</label>
                  <input value={form.codigo} onChange={(e) => setForm({ ...form, codigo: e.target.value })}
                    className="mt-1 w-full rounded-lg border border-slate-600 bg-slate-900 px-3 py-2 text-sm text-slate-100 placeholder:text-slate-500 outline-none focus:ring-2 focus:ring-sky-500" placeholder="CT-001" /></div>
              </div>

 ...
 *
 *  · `/api/taller-operator/recepciones/*` la usa la APK del patio, con las
 *    credenciales de operario que ya existen (`x-operator-name` /
 *    `x-operator-pin`). No se inventa un quinto modelo de autenticación.
 *  · `/api/recepciones/*` la usa WorkPlanner en el navegador, con el guarda
 *    de supervisor del panel.
 *
...
export type DependenciasRecepciones = {
...
export function createRecepcionesRouter(dep: DependenciasRecepciones): Router {
  const r = Router();
   ...
   */
  r.get("/taller-operator/recepciones/catalogo", requireTallerOperator, async (req, res) => {
    try {
   ...
   */
  r.get("/taller-operator/recepciones/vehiculo", requireTallerOperator, async (req, res) => {
    try {
  ...
  r.post("/taller-operator/recepciones", requireTallerOperator, async (req, res) => {
    try {
  ...
  r.get("/taller-operator/recepciones/mias", requireTallerOperator, async (req, res) => {
    try {
      const { techName } = (req as any).roadsideOperator as { techName: string };
      const filas = await db.query(
  ...
  r.post(
    "/taller-operator/recepciones/:id/fotos",
    requireTallerOperator,
    upload.single("file"),
    async (req, res) => {
      try {
   ...
   */
  r.post("/taller-operator/recepciones/ocr-matricula", requireTallerOperator, async (req, res) => {
    try {
  ...
  /** La bandeja. */
  r.get("/recepciones", requireSupervisorRole, async (req, res) => {
    try {
  ...
  r.get("/recepciones/:id", requireSupervisorRole, async (req, res) => {
    try {
      const id = Number(req.params.id);
      if (!Number.isFinite(id)) return res.status(400).json({ error: "ID no válido" });
      const filas = await db.query(
   ...
   */
  r.put("/recepciones/:id", requireSupervisorRole, async (req, res) => {
    try {
      const id = Number(req.params.id);
      if (!Number.isFinite(id)) return res.status(400).json({ error: "ID no válido" });
      const body = (req.body ?? {}) as Record<string, unknown>;
   ...
   */
  r.post("/recepciones/:id/convertir", requireSupervisorRole, async (req, res) => {
  ...
  r.post("/recepciones/:id/descartar", requireSupervisorRole, async (req, res) => {
    try {
      const id = Number(req.params.id);
      if (!Number.isFinite(id)) return res.status(400).json({ error: "ID no válido" });

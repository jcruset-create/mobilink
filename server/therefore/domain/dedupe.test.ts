...
function correo(cambios: Partial<CorreoNormalizado> = {}): CorreoNormalizado {
  return {
    ...
    enRespuestaA: null,
    hashesAdjuntos: [],
    actuaciones: [{ accion: "GRABAR", albaranNormalizado: "2028359553" }],
    urgente: false,
    tareaVencida: false,
    reclamacion: false,
    ...cambios,
...
function expediente(cambios: Partial<ExpedienteCandidato> = {}): ExpedienteCandidato {
  return {
    ...
    actuaciones: [
      { id: "act-1", accion: "GRABAR", albaranNormalizado: "2028359553", descartada: false },
    ],
    hashesAdjuntos: [],
    hilos: [],
    messageIds: [],
...
describe("normalizar identificadores", () => {
  it("la misma factura escrita con y sin ceros es la misma", () => {
    expect(normalizarIdentificador("0000123514")).toBe("123514");
    expect(normalizarIdentificador("123514")).toBe("123514");
    expect(normalizarIdentificador("123.514")).toBe("123514");
  });
...
describe("puntuación", () => {
  ...
  it("el mismo albarán suma una vez aunque coincidan varios", () => {
    ...
    const varios = puntuar(
      correo({
        actuaciones: [
          { accion: "GRABAR", albaranNormalizado: "2028359553" },
          { accion: "GRABAR", albaranNormalizado: "806295" },
        ],
      }),
      expediente({
        actuaciones: [
          { id: "a", accion: "GRABAR", albaranNormalizado: "2028359553", descartada: false },
          { id: "b", accion: "GRABAR", albaranNormalizado: "806295", descartada: false },
        ],
      })
    );
  ...
  it("una actuación descartada ya no cuenta como albarán del expediente", () => {
    const c = puntuar(
      ...
      expediente({
        actuaciones: [
          { id: "act-1", accion: "GRABAR", albaranNormalizado: "2028359553", descartada: true },
        ],
      })
    );
  ...
  it("cada motivo lleva su texto para la pantalla de revisión", () => {
    ...
    expect(c.motivos.map((m) => m.texto)).toContain("misma factura 0000123514");
    expect(c.motivos.map((m) => m.texto)).toContain("albarán 2028359553 ya en INC-000452");
  });
});
...
describe("las decisiones que pide el encargo", () => {
  ...
  it("caso 5 · GRABAR y luego MODIFICAR el mismo albarán siguen siendo un expediente", () => {
    const r = deduplicar(
      correo({ actuaciones: [{ accion: "MODIFICAR", albaranNormalizado: "2028359553" }] }),
      [expediente()]
    );
    expect(r.decision).toBe("FUSIONAR");
  });

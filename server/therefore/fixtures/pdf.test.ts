...
async function facturaDePrueba(): Promise<Buffer> {
  ...
  const lineas: [string, number][] = [
    ...
    ["Albaran: ENT-100126-0806295   Fecha: 24/08/2026", 770],
    ["Referencia      Descripcion            Cant.  Precio  Dto.       Importe", 745],
    ["1300986494235   PASTILLA FRENO DELT.    1,00   81,10  60% + 10%    29,20", 725],
    ["1300986479387   DISCO DE FRENO TRAS.    2,00  175,00  40%         210,00", 705],
    ["Matricula: 6352GVV   Bastidor: VF30E9HZHAS115406", 675],
    ["Albaran: ENT-100126-0806296   Fecha: 25/08/2026", 640],
    ["1300986494640   PASTILLA FRENO TRAS.    1,00   93,40  60% + 10%    33,62", 620],
    ["Total factura                                                    272,82", 80],
  ];
...
describe("redibujar", () => {
  ...
  it("el orden vertical se mantiene", async () => {
    ...
    expect(y("PROVEEDOR EJEMPLO")).toBeLessThan(y("ENT-100126-0806295"));
    expect(y("ENT-100126-0806295")).toBeLessThan(y("Total factura"));
  });
});
...
describe("redibujar anonimizando", () => {
  ...
  it("quita lo identificativo y no toca ni un importe", async () => {
    ...
    const mapa = {
      ...
      "6352GVV": matriculaFalsa("6352GVV"),
      "VF30E9HZHAS115406": "SALVLBRVMVK2CXAJK",
      B12345674: "B00000000",
    };
    ...
    expect(texto).not.toContain("6352GVV");
    expect(texto).not.toContain("VF30E9HZHAS115406");

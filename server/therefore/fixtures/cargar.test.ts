...
describe("suma de líneas", () => {
  it("suma en céntimos", () => {
    expect(sumaDeLineas([linea({ importeCentimos: 2920 }), linea({ importeCentimos: 21000 })])).toBe(
      23920
    );
  });
...
describe("aritmética de una línea", () => {
  /*
   * El caso del encargo: 81,10 con 60 % + 10 % da 29,196, que redondea a
   * 29,20. Es lo que permite corroborar cinco campos entre sí en vez de
   * creerse cada uno por separado.
   */
  it("aplica los descuentos encadenados en orden y cuadra", () => {
    const l = linea({
      cantidad: 1,
      precioUnitarioCentimos: 8110,
      descuentos: [
        { orden: 1, porcentaje: 60, raw: "60%" },
        { orden: 2, porcentaje: 10, raw: "10%" },
      ],
      importeCentimos: 2920,
    });
  ...
  it("un descuento único también", () => {
    const l = linea({
      cantidad: 2,
      precioUnitarioCentimos: 17500,
      descuentos: [{ orden: 1, porcentaje: 40, raw: "40%" }],
      importeCentimos: 21000,
    });
    expect(descuadreDeLinea(l)).toBe(0);
  });

-- ============================================================
-- Mobilink SaaS — módulo «or-manuales» como licencia y como acceso de usuario.
--
-- Sin esto, `requireModule("or-manuales")` contesta 403 a todo el mundo: el
...
-- la lista completa de módulos, incluido «or-manuales». Idempotente.
--
-- Mismo molde que saas_modulo_recepciones.sql.
--
...
--   · registrarModuloOrManuales en server/or-manuales/schema.ts
--
-- Los tres llevan ya «or-manuales».
-- ============================================================
...
begin
  ...
  select '00000000-0000-4000-a000-000000000001', 'or-manuales'
  where not exists (
    select 1 from app_licencias
     where empresa_id = '00000000-0000-4000-a000-000000000001'
       and modulo = 'or-manuales'
  );

  raise notice 'OK: módulos admitidos → %', v_lista;
  raise notice 'Recuerda dar acceso a cada usuario: insert into app_usuario_modulos (user_id, modulo, rol) values (..., ''or-manuales'', ''gestor'')';
end
$migracion$;

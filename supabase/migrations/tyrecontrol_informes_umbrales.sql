-- ============================================================
-- SEA TyreControl — Umbrales de profundidad configurables por empresa
-- Sustituye los umbrales fijos (1,6 / 3,0 mm) de las funciones de Informes
-- por los definidos por empresa en tc_config_umbrales, con fallback a los
-- valores legales por defecto cuando la empresa no tiene fila propia.
-- ============================================================
...
create policy tc_config_umbrales_select on public.tc_config_umbrales
...
create policy tc_config_umbrales_write on public.tc_config_umbrales
  for all using ( tc_is_superadmin() or tc_is_admin() )
  with check ( tc_is_superadmin() or tc_is_admin() );

-- ── KPIs (umbrales por empresa) ──────────────────────────────
...
as $$
  ...
  ult as (
    select distinct on (d.neumatico_id) d.neumatico_id, d.profundidad_mm, d.empresa_id
  ...
  ultu as (
    select u.neumatico_id, u.profundidad_mm,
           coalesce(cu.profundidad_minima_mm, 1.6) as minmm,
           coalesce(cu.profundidad_aviso_mm, 3.0) as avisomm
    from ult u
    left join tc_config_umbrales cu on cu.empresa_id = u.empresa_id
  )
...
-- ── Estado de flota (umbrales por empresa) ───────────────────
  ...
  clasif as (
    select v.id,
      case
        ...
        when mp.minmm <= coalesce(cu.profundidad_minima_mm, 1.6) then 'urgente'
        when mp.minmm <= coalesce(cu.profundidad_aviso_mm, 3.0) then 'revisar'
        else 'correcto'
      end as estado
    from veh v
    left join ultrev ur on ur.vehiculo_id = v.id
    left join minprof mp on mp.vehiculo_id = v.id
    left join tc_config_umbrales cu on cu.empresa_id = v.empresa_id
  )
  select json_build_object(

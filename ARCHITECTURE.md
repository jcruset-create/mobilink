...
- **El buzón se lee sin escribir en él.** El buzón (`pedidos@comercialsea.com`,
  en cdmon) es dedicado, y aun así el módulo **no toca ninguna bandera**: el
  avance va en `rcp_config` (`buzon.progreso.<carpeta>`: hasta qué UID se miró,
  con el UIDVALIDITY de la carpeta). Con `\Seen` el progreso dependería de un
  estado que puede cambiar cualquiera que abra el webmail —un vistazo deja
  «leído» lo no procesado, y un «marcar como no leído» lo reprocesa entero—;
  el UID sólo lo mueve este módulo. Si el servidor renumera, la marca se
  descarta y se vuelve a mirar desde la activación, que es inofensivo porque
  el Message-ID reconoce lo ya procesado. Un correo que falla no deja avanzar
  la marca.
- **Los remitentes admitidos viven en la ficha del proveedor**
  ...
  a un buzón dedicado también le llega publicidad, y guardarla sería meter en
  la base el cuerpo de correos que no son de nadie de aquí. El `.eml`
  importado a mano sí se acepta sin lista, porque lo trae una persona a
  propósito.
